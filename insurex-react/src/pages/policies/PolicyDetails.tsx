...
import {
  ...
  CircularProgress, Alert, Card, CardContent, Table, TableBody, TableCell, TableContainer,
  TableHead, TableRow, IconButton, Tooltip, Dialog, DialogTitle, DialogContent, DialogActions, TextField,
} from '@mui/material';
import { Edit, Delete, ArrowBack, Renew, Cancel, PlayArrow, Pause } from '@mui/icons-material';
...
export const PolicyDetails: React.FC = () => {
  ...
  useEffect(() => { if (id) fetchPolicy(id); }, [id]);
  ...
  const handleCancel = async () => {
    if (!id) return;
    try {
      await policyService.cancelPolicy(id, { reason: cancelReason, cancellationDate: new Date().toISOString() });
  ...
  const handleRenew = async () => {
    if (!id) return;
    try {
      await policyService.renewPolicy(id, { newEndDate });
  ...
  if (error || !policy) return (<Alert severity="error" sx={{ mb: 2 }}>{error || 'Policy not found'}</Alert>);
  ...
  return (
    <Box>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        ...
        <Box sx={{ display: 'flex', gap: 1 }}>
          {policy.status === 'pending' && (
            <Button variant="contained" color="success" startIcon={<PlayArrow />} onClick={handleActivate}>Activate</Button>
          )}
          {policy.status === 'active' && (
            <Button variant="outlined" color="warning" startIcon={<Pause />} onClick={handleSuspend}>Suspend</Button>
          )}
          {(policy.status === 'active' || policy.status === 'suspended') && (
            <Button variant="outlined" startIcon={<Renew />} onClick={() => setRenewDialogOpen(true)}>Renew</Button>
          )}
          ...
          <Paper sx={{ p: 3, mb: 3 }}>
            ...
            <Grid container spacing={3}>
              ...
              <Grid item xs={12} sm={6}>
                ...
                <Typography variant="body1">{formatCurrency(policy.premium, policy.currency)}</Typography>
              </Grid>
              <Grid item xs={12} sm={6}>
                <Typography variant="subtitle2" color="textSecondary">Coverage Amount</Typography>
                <Typography variant="body1">{formatCurrency(policy.coverageAmount, policy.currency)}</Typography>
              </Grid>
              <Grid item xs={12} sm={6}>
                <Typography variant="subtitle2" color="textSecondary">Deductible</Typography>
                <Typography variant="body1">{formatCurrency(policy.deductible, policy.currency)}</Typography>
              </Grid>
              <Grid item xs={12} sm={6}>
                <Typography variant="subtitle2" color="textSecondary">Payment Frequency</Typography>
                <Typography variant="body1" sx={{ textTransform: 'capitalize' }}>{policy.paymentFrequency}</Typography>
              </Grid>
            </Grid>
            {policy.description && (
              <>
        ...
        <Grid item xs={12} md={4}>
          <Card sx={{ mb: 3 }}>
            <CardContent>
              ...
              <List dense>
                <ListItem><ListItemText primary="Total Premium" secondary={formatCurrency(policy.premium, policy.currency)} /></ListItem>
                <ListItem><ListItemText primary="Total Coverage" secondary={formatCurrency(policy.coverageAmount, policy.currency)} /></ListItem>
                <ListItem><ListItemText primary="Created" secondary={formatDate(policy.createdAt)} /></ListItem>
      ...
      <Dialog open={cancelDialogOpen} onClose={() => setCancelDialogOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <TextField autoFocus margin="dense" label="Cancellation Reason" fullWidth multiline rows={3} value={cancelReason} onChange={(e) => setCancelReason(e.target.value)} />
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setCancelDialogOpen(false)}>Cancel</Button>
          <Button onClick={handleCancel} color="error" variant="contained">Confirm Cancellation</Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={renewDialogOpen} onClose={() => setRenewDialogOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <TextField margin="dense" label="New End Date" type="date" fullWidth value={newEndDate} onChange={(e) => setNewEndDate(e.target.value)} InputLabelProps={{ shrink: true }} />
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setRenewDialogOpen(false)}>Cancel</Button>
          <Button onClick={handleRenew} variant="contained">Renew Policy</Button>
        </DialogActions>
      </Dialog>
    </Box>
  );
